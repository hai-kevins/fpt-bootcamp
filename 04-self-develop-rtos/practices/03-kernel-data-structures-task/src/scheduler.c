...
void rtos_scheduler_on_tick(void)
{
    ...
    for (size_t i = 0U; i < rtos_task_registry_count(); ++i)
    {
        ...
        if ((task != (rtos_task_t *)0) && (task->state == RTOS_TASK_BLOCKED) &&
            (task->wake_tick != RTOS_WAIT_FOREVER) && tick_due(g_kernel_tick, task->wake_tick))
        {
...
{
    if (ticks == 0U) { rtos_task_yield(); return; }
    const rtos_irq_state_t irq_state = rtos_critical_enter();
...
{
    if (!rtos_task_registry_validate() || !rtos_ready_validate()) { return false; }
    if (g_current_task == (rtos_task_t *)0) { return true; }
    return rtos_task_is_valid(g_current_task) &&
           ((g_current_task->state == RTOS_TASK_RUNNING) ||
            (g_current_task->state == RTOS_TASK_BLOCKED));
}
