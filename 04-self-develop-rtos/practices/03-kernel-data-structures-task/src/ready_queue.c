...
{
    for (size_t i = 0U; i < RTOS_PRIORITY_COUNT; ++i)
    {
        rtos_list_init(&g_ready_queues[i]);
    }
    g_ready_bitmap = 0U;
}
...
bool rtos_ready_insert(rtos_task_t *task)
{
    ...
    if (!rtos_task_is_valid(task) || (p >= RTOS_PRIORITY_COUNT) || (task->ready_node.owner != (const void *)0))
    {
        return false;
    }
    if (!rtos_list_push_back(&g_ready_queues[p], &task->ready_node))
    {
        return false;
    }
...
{
    const uint8_t p = (task != (rtos_task_t *)0) ? task->effective_priority : 0xFFU;
    if (!rtos_task_is_valid(task) || (p >= RTOS_PRIORITY_COUNT) || (task->ready_node.owner != &g_ready_queues[p]))
    {
        return false;
    }
    if (!rtos_list_remove(&g_ready_queues[p], &task->ready_node))
    {
        return false;
    }
    if (rtos_list_is_empty(&g_ready_queues[p]))
    {
        g_ready_bitmap &= ~ (1UL << p);
    }
    return true;
...
bool rtos_ready_rotate(uint8_t priority)
{
    ...
    if (rtos_list_count(&g_ready_queues[priority]) <= 1U)
    {
        return true;
    }
...
bool rtos_ready_validate(void)
{
    ...
    for (uint8_t p = 0U; p < RTOS_PRIORITY_COUNT; ++p)
    {
        ...
        for (rtos_list_node_t *n = list->head; n != (rtos_list_node_t *)0; n = n->next)
        {
            ...
            if (!rtos_task_is_valid(task) || (task->effective_priority != p) || ((task->state != RTOS_TASK_READY)
