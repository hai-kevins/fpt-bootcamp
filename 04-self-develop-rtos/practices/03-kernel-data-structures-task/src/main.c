...
static void worker_a(void *argument)
{
    ...
    for (;;)
    {
        ...
        if ((local & 0x1FFFUL) == 0U) { rtos_task_delay(15U); }
    }
}
...
static void worker_b(void *argument)
{
    ...
    for (;;)
    {
        ...
        if ((local & 0x1FFFUL) == 0U) { rtos_task_delay(25U); }
    }
}
...
static void monitor(void *argument)
{
    ...
    for (;;)
    {
        if (uart1_byte_available())
        {
            ...
            if (command == (uint8_t)'h') { rtos_task_inspector_print_help(); }
            else if (command == (uint8_t)'t') { rtos_task_inspector_print_tasks(); }
            else if (command == (uint8_t)'r') { rtos_task_inspector_print_ready(); }
            else if (command == (uint8_t)'c') { rtos_task_inspector_print_current(); }
            else if (command == (uint8_t)'s') { rtos_task_inspector_print_stacks(); }
            else if (command == (uint8_t)'v') { rtos_task_inspector_print_validation(); }
...
{
    (void)argument;
    for (;;) { ++g_idle_counter; rtos_port_wait_for_interrupt(); }
}

static void create_tasks(void)
{
    RTOS_ASSERT(rtos_task_create_static(&g_worker_a_task, "worker-a", 0U, 1U, worker_a,
                                    (void *)0, g_worker_a_stack, STACK_WORDS_WORKER));
    RTOS_ASSERT(rtos_task_create_static(&g_worker_b_task, "worker-b", 1U, 1U, worker_b,
                                    (void *)0, g_worker_b_stack, STACK_WORDS_WORKER));
    RTOS_ASSERT(rtos_task_create_static(&g_monitor_task, "monitor", 2U, 2U, monitor,
                                    (void *)0, g_monitor_stack, STACK_WORDS_MONITOR));
    RTOS_ASSERT(rtos_task_create_static(&g_idle_task, "idle", 3U, 3U, idle,
