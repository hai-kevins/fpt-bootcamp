...
{
    return (node != (const rtos_list_node_t *)0) && (node->owner == (const void *)0) && (node->previous == (rtos_list_node_t *)0) && (node->next == (rtos_list_node_t *)0);
}

void rtos_list_init(rtos_list_t *list)
{
    if (list == (rtos_list_t *)0)
    {
        return;
    }
...
{
    if ((list == (rtos_list_t *)0) || !node_is_detached(node))
    {
        return false;
    }
    node->owner = list;
    node->previous = (rtos_list_node_t *)0;
    node->next = list->head;
    if (list->head != (rtos_list_node_t *)0)
    {
...
bool rtos_list_push_back(rtos_list_t *list, rtos_list_node_t *node)
{
    ...
    if (list->tail != (rtos_list_node_t *)0)
    {
        list->tail->next = node;
    }
    else
...
{
    if ((list == (rtos_list_t *)0) || (position == (rtos_list_node_t *)0) || (position->owner != list) || !node_is_detached(node))
    {
        return false;
    }
    if (position == list->head)
    {
        return rtos_list_push_front(list, node);
    }
    node->owner = list;
...
bool rtos_list_insert_after(rtos_list_t *list, rtos_list_node_t *position, rtos_list_node_t *node)
{
    ...
    {
        return rtos_list_push_back(list, node);
    }
    node->owner = list;
...
{
    if ((list == (rtos_list_t *)0) || (node == (rtos_list_node_t *)0) || (node->owner != list) || (list->count == 0U))
    {
        return false;
    }
    if (node->previous != (rtos_list_node_t *)0)
    {
    ...
    if (node->next != (rtos_list_node_t *)0)
    {
...
rtos_list_node_t *rtos_list_pop_front(rtos_list_t *list)
{
    ...
    if (node != (rtos_list_node_t *)0)
    {
        (void) rtos_list_remove(list, node);
    }
    return node;
}
...
bool rtos_list_validate(const rtos_list_t *list)
{
    ...
    if (list == (const rtos_list_t *)0)
    {
        return false;
    }
    if (list->count == 0U)
    {
        return (list->head == (rtos_list_node_t *)0) && (list->tail == (rtos_list_node_t *)0);
    }
    if ((list->head == (rtos_list_node_t *)0) || (list->tail == (rtos_list_node_t *)0) || (list->head->previous != (rtos_list_node_t *)0)
        || (list->tail->next != (rtos_list_node_t *)0))
    {
        return false;
    }
    ...
    slow = list->head;
    fast = list->head;
    while ((fast != (const rtos_list_node_t *)0) && (fast->next != (rtos_list_node_t *)0))
    {
        slow = slow->next;
        fast = fast->next->next;
        if (slow == fast)
