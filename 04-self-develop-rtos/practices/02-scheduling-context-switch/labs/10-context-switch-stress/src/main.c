...
static void high(void *x)
{
    ...
    {
        ++vh;
        if (!rtos_task_stack_guard_ok(&high_t) ||
            !rtos_task_stack_guard_ok(&a) ||
            !rtos_task_stack_guard_ok(&b) ||
            !rtos_task_stack_guard_ok(&c) ||
            !rtos_task_stack_guard_ok(&idle_t))
        {
            ++guard_failures;
...
static void idle(void *x){(void)x;for(;;){rtos_port_wait_for_interrupt();}}
...
int main(void)
{
    ...
    RTOS_ASSERT(rtos_scheduler_add_task(&high_t)); RTOS_ASSERT(rtos_scheduler_add_task(&a));
    RTOS_ASSERT(rtos_scheduler_add_task(&b)); RTOS_ASSERT(rtos_scheduler_add_task(&c));
