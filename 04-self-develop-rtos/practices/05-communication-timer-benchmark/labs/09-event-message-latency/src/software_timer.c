...
bool hr_software_timer_init(hr_software_timer_t *timer,
                            ...
                            void *argument)
{
    if ((timer == (hr_software_timer_t *)0) || (name == (const char *)0) ||
        (callback == (hr_timer_callback_t)0)) { return false; }
                             ...
                             uint32_t period_ticks)
{
    ...
    if ((timer == (hr_software_timer_t *)0) || (delay_ticks == 0U) ||
        (timer->state == HR_TIMER_CALLBACK_PENDING))
    {
    ...
    if (!ok) { timer->state = HR_TIMER_INACTIVE; }
    hr_critical_exit(state);
    return ok;
}
...
void hr_software_timer_complete(hr_software_timer_t *timer, uint32_t now)
{
    ...
    if (timer->periodic && (timer->period_ticks > 0U))
    {
        ...
        timer->state = HR_TIMER_ACTIVE;
        if (!insert_locked(timer)) { timer->state = HR_TIMER_INACTIVE; }
    }
    else
    {
        timer->state = HR_TIMER_INACTIVE;
...
size_t hr_software_timer_active_count(void) { return hr_list_count(&g_active_timers); }
const hr_list_t *hr_software_timer_active_list(void) { return &g_active_timers; }
...
bool hr_software_timer_validate(void)
{
    ...
    if (!hr_list_validate(&g_active_timers)) { return false; }
    node = g_active_timers.head;
    while (node != (const hr_list_node_t *)0)
    {
        const hr_software_timer_t *timer = HR_CONTAINER_OF(node, hr_software_timer_t, node);
        if ((timer->name == (const char *)0) || (timer->callback == (hr_timer_callback_t)0) ||
            (timer->state != HR_TIMER_ACTIVE)) { return false; }
        if ((previous != (const hr_software_timer_t *)0) &&
            hr_tick_before(timer->expiry_tick, previous->expiry_tick)) { return false; }
        previous = timer;
        node = node->next;
    }
    return true;
