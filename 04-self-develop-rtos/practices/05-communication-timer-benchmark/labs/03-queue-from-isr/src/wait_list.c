...
{
    hr_list_node_t *node;
    if ((list == (hr_list_t *)0) || !hr_task_is_valid(task) || (task->wait_node.owner != (const void *)0))
    {
        return false;
    }
...
{
    return (list != (hr_list_t *)0) && hr_task_is_valid(task) && hr_list_remove(list, &task->wait_node);
}

hr_task_t *hr_wait_list_front(const hr_list_t *list)
{
    hr_list_node_t *node = hr_list_front(list);
    return (node != (hr_list_node_t *)0) ? HR_CONTAINER_OF(node, hr_task_t, wait_node) : (hr_task_t *)0;
}
