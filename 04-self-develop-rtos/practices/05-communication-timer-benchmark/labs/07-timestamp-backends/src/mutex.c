...
static void recompute_task_priority_locked(hr_task_t *task)
{
    uint8_t effective;
    hr_list_node_t *node;
    if (!hr_task_is_valid(task)) { return; }
...
{
    if ((mutex == (hr_mutex_t *)0) || (name == (const char *)0)) { return false; }
    mutex->name = name;
...
hr_wait_result_t hr_mutex_lock(hr_mutex_t *mutex, uint32_t timeout_ticks)
{
    ...
    {
        hr_critical_exit(state);
        return HR_WAIT_CANCELLED; /* Non-recursive mutex. */
    }
    ...
    {
        (void)hr_scheduler_set_effective_priority_locked(mutex->owner,
                                                         current->effective_priority);
    }
    if (!hr_scheduler_block_current_locked(mutex, &mutex->waiters,
                                           HR_WAIT_KIND_MUTEX,
                                           timeout_ticks))
    {
        hr_critical_exit(state);
...
{
    if ((mutex == (const hr_mutex_t *)0) || (mutex->name == (const char *)0) ||
        !hr_wait_list_validate(&mutex->waiters)) { return false; }
    if (mutex->owner == (const hr_task_t *)0)
    {
        return mutex->owner_node.owner == (const void *)0;
    }
    return hr_task_is_valid(mutex->owner) &&
