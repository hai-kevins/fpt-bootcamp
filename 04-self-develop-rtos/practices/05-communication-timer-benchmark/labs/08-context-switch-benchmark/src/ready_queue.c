...
{
    for (size_t i = 0U; i < HR_PRIORITY_COUNT; ++i)
    {
        hr_list_init(&g_ready_queues[i]);
    }
    g_ready_bitmap = 0U;
}
...
bool hr_ready_insert(hr_task_t *task)
{
    ...
    if (!hr_task_is_valid(task) || (p >= HR_PRIORITY_COUNT) || (task->ready_node.owner != (const void *)0))
    {
        return false;
    }
    if (!hr_list_push_back(&g_ready_queues[p], &task->ready_node))
    {
        return false;
    }
...
{
    const uint8_t p = (task != (hr_task_t *)0) ? task->effective_priority : 0xFFU;
    if (!hr_task_is_valid(task) || (p >= HR_PRIORITY_COUNT) || (task->ready_node.owner != &g_ready_queues[p]))
    {
        return false;
    }
    if (!hr_list_remove(&g_ready_queues[p], &task->ready_node))
    {
        return false;
    }
    if (hr_list_is_empty(&g_ready_queues[p]))
    {
        g_ready_bitmap &= ~ (1UL << p);
    }
    return true;
...
bool hr_ready_rotate(uint8_t priority)
{
    ...
    if (hr_list_count(&g_ready_queues[priority]) <= 1U)
    {
        return true;
    }
...
bool hr_ready_validate(void)
{
    ...
    for (uint8_t p = 0U; p < HR_PRIORITY_COUNT; ++p)
    {
        ...
        for (hr_list_node_t *n = list->head; n != (hr_list_node_t *)0; n = n->next)
        {
            ...
            if (!hr_task_is_valid(task) || (task->effective_priority != p) || ((task->state != HR_TASK_READY)
                && (task->state != HR_TASK_RUNNING)))
            {
                return false;
            }
        }
