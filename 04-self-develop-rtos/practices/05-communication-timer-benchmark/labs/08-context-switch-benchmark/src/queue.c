...
static bool higher_than_current(const hr_task_t *task)
{
    ...
    return (task != (const hr_task_t *)0) && ((current == (const hr_task_t *)0) || (task->effective_priority < current->effective_priority));
}

bool hr_queue_init_static(hr_queue_t *queue, const char *name, void *storage, size_t capacity, size_t item_size)
{
    if ((queue == (hr_queue_t *)0) || (name == (const char *)0) || (storage == (void *)0) || (capacity == 0U)
        || (item_size == 0U) || (capacity > (SIZE_MAX / item_size)))
    {
        return false;
    }
    queue->name = name;
...
hr_wait_result_t hr_queue_send(hr_queue_t *queue, const void *item, uint32_t timeout_ticks)
{
    ...
    if (!hr_scheduler_block_current_locked(queue, &queue->send_waiters, HR_WAIT_KIND_QUEUE_SEND, timeout_ticks))
    {
...
hr_wait_result_t hr_queue_receive(hr_queue_t *queue, void *item, uint32_t timeout_ticks)
{
    ...
    current->wait_send_item = (const void *)0;
    if (!hr_scheduler_block_current_locked(queue, &queue->receive_waiters, HR_WAIT_KIND_QUEUE_RECEIVE, timeout_ticks))
    {
        current->wait_receive_buffer = (void *)0;
        hr_critical_exit(state);
        return HR_WAIT_CANCELLED;
...
bool hr_queue_send_from_isr(hr_queue_t *queue, const void *item, bool *higher_priority_task_woken)
{
    ...
    {
        receiver = hr_wait_list_front(&queue->receive_waiters);
        if ((receiver != (hr_task_t *)0) && (receiver->wait_receive_buffer != (void *)0))
        {
            (void) memcpy(receiver->wait_receive_buffer, item, queue->item_size);
            ++queue->send_count;
            ++queue->receive_count;
...
{
    if ((queue == (const hr_queue_t *)0) || (queue->name == (const char *)0) || (queue->storage == (uint8_t *)0)
        || (queue->capacity == 0U) || (queue->item_size == 0U) || (queue->head >= queue->capacity) || (queue->tail >= queue->capacity)
        || (queue->count > queue->capacity))
    {
        return false;
    }
    return hr_wait_list_validate(&queue->send_waiters) && hr_wait_list_validate(&queue->receive_waiters);
}
