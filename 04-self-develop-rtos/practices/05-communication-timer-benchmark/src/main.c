...
static uint8_t g_pool_memory[MESSAGE_POOL_BLOCKS * MESSAGE_POOL_BLOCK_SIZE]
...
static void producer_task(void *argument)
{
    ...
    for (;;)
    {
        ...
        if ((sequence & 0x0FUL) == 0U)
        {
            ...
            if (block != (void *)0) { (void)hr_message_pool_free(&g_message_pool, block); }
        }
...
static void consumer_task(void *argument)
{
    ...
    for (;;)
    {
        ...
        {
            hr_benchmark_stats_add(&g_message_stats,
                                   hr_benchmark_elapsed(message.timestamp,
                                                        hr_benchmark_now()));
        }
    }
}
...
static void switch_b_task(void *argument)
{
    ...
    for (;;)
    {
        if (g_switch_turn == 1U)
        {
            hr_benchmark_stats_add(&g_switch_stats,
                                   hr_benchmark_elapsed(g_switch_start,
                                                        hr_benchmark_now()));
            g_switch_turn = 0U;
            hr_task_yield();
        }
...
static void monitor_task(void *argument)
{
    ...
    for (;;)
    {
        ...
        if ((g_one_shot_timer.state == HR_TIMER_INACTIVE) &&
            ((hr_scheduler_tick_now() % 2000U) < 25U))
        {
...
{
    (void)argument;
    for (;;) { hr_port_wait_for_interrupt(); }
}

static void create_tasks(void)
{
    HR_ASSERT(hr_task_create_static(&g_timer_service_task, "timer-service", 0U, 0U,
                                    hr_timer_service_task, (void *)0,
                                    g_timer_service_stack, STACK_MEDIUM));
    HR_ASSERT(hr_task_create_static(&g_consumer_task, "consumer", 1U, 1U,
                                    consumer_task, (void *)0,
                                    g_consumer_stack, STACK_MEDIUM));
    HR_ASSERT(hr_task_create_static(&g_switch_a_task, "switch-a", 2U, 1U,
                                    switch_a_task, (void *)0,
                                    g_switch_a_stack, STACK_SMALL));
    HR_ASSERT(hr_task_create_static(&g_switch_b_task, "switch-b", 3U, 1U,
                                    switch_b_task, (void *)0,
                                    g_switch_b_stack, STACK_SMALL));
    HR_ASSERT(hr_task_create_static(&g_producer_task, "producer", 4U, 2U,
                                    producer_task, (void *)0,
                                    g_producer_stack, STACK_SMALL));
    HR_ASSERT(hr_task_create_static(&g_monitor_task, "monitor", 5U, 2U,
                                    monitor_task, (void *)0,
                                    g_monitor_stack, STACK_MONITOR));
    HR_ASSERT(hr_task_create_static(&g_idle_task, "idle", 6U, 3U,
                                    idle_task, (void *)0,
                                    g_idle_stack, STACK_IDLE));
...
int main(void)
{
    ...
    HR_ASSERT(hr_queue_init_static(&g_message_queue, "messages",
                                   g_queue_storage, MESSAGE_QUEUE_CAPACITY,
                                   sizeof(app_message_t)));
    HR_ASSERT(hr_message_pool_init_static(&g_message_pool, "message-pool",
                                          g_pool_memory, MESSAGE_POOL_BLOCK_SIZE,
    ...
    HR_ASSERT(hr_software_timer_init(&g_one_shot_timer, "one-shot",
                                     one_shot_callback, (void *)0));
    HR_ASSERT(hr_software_timer_init(&g_periodic_timer, "periodic",
                                     periodic_callback, (void *)0));
    ...
    hr_benchmark_console_bind(&g_message_queue, &g_message_pool,
                              &g_one_shot_timer, &g_periodic_timer,
                              &g_switch_stats, &g_message_stats, &g_timer_stats);
