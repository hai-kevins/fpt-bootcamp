...
{
    if ((semaphore == (rtos_semaphore_t *)0) || (name == (const char *)0) || (max_count == 0U) || (initial_count > max_count))
    {
        return false;
    }
...
rtos_wait_result_t rtos_semaphore_take(rtos_semaphore_t *semaphore, uint32_t timeout_ticks)
{
    ...
    if (!rtos_scheduler_block_current_locked(semaphore, &semaphore->waiters, RTOS_WAIT_KIND_SEMAPHORE, timeout_ticks))
    {
        rtos_critical_exit(state);
        return RTOS_WAIT_CANCELLED;
    }
...
bool rtos_semaphore_give_from_isr(rtos_semaphore_t *semaphore, bool *higher_priority_task_woken)
{
    ...
    if (higher_priority_task_woken != (bool *)0)
    {
        ...
        *higher_priority_task_woken = ok && (woken != (rtos_task_t *)0) && ((current == (const rtos_task_t *)0)
...
{
    return (semaphore != (const rtos_semaphore_t *)0) && (semaphore->name != (const char *)0) && (semaphore->max_count > 0U) && (semaphore->count <= semaphore->max_count) && rtos_wait_list_validate(&semaphore->waiters);
}
