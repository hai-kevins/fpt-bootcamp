...
{
    return (task != (const rtos_task_t *)0) && ((g_current_task == (rtos_task_t *)0) || (task->effective_priority < g_current_task->effective_priority));
}
...
bool rtos_scheduler_wake_task_locked(rtos_task_t *task, rtos_wait_result_t result)
{
    ...
    if (task->wait_node.owner != (const void *)0)
    {
        ...
        if (!rtos_list_remove(list, &task->wait_node))
        {
            return false;
        }
    }
...
bool rtos_scheduler_block_current_locked(void *object, rtos_list_t *waiters, rtos_wait_kind_t kind, uint32_t timeout_ticks)
{
    ...
    if (!rtos_task_is_valid(task) || (task->state != RTOS_TASK_RUNNING) || (task->name[0] == 'i' && task->name[1] == 'd'
...
static void process_timeouts(void)
{
    ...
    while ((task = rtos_timeout_take_due(g_kernel_tick)) != (rtos_task_t *)0)
    {
        ...
        if (kind == RTOS_WAIT_KIND_MUTEX)
        {
            rtos_mutex_waiter_removed_locked(object);
        }
        if (priority_higher_than_current(task))
        {
            rtos_port_request_context_switch();
...
rtos_wait_result_t rtos_task_delay(uint32_t ticks)
{
    ...
    task = g_current_task;
    if (!rtos_scheduler_block_current_locked((void *)0, (rtos_list_t *)0, RTOS_WAIT_KIND_DELAY, ticks))
    {
        rtos_critical_exit(irq_state);
...
{
    bool linked;
    if (!rtos_task_is_valid(task) || (priority >= RTOS_PRIORITY_COUNT))
    {
        return false;
    }
    ...
    if (linked && !rtos_ready_remove(task))
    {
        return false;
    }
    task->effective_priority = priority;
    if (linked && !rtos_ready_insert(task))
    {
        return false;
    }
    if (priority_higher_than_current(task))
...
{
    const rtos_irq_state_t irq_state = rtos_critical_enter();
    if (!rtos_task_is_valid(task) || (task->state == RTOS_TASK_SUSPENDED) || (task->name[0] == 'i' && task->name[1] == 'd'
        && task->name[2] == 'l' && task->name[3] == 'e'))
    {
        rtos_critical_exit(irq_state);
        return false;
    ...
    if (task->wait_kind == RTOS_WAIT_KIND_MUTEX)
    {
        rtos_mutex_waiter_removed_locked(task->waiting_object);
    }
    task->waiting_object = (void *)0;
    task->waiting_list = (rtos_list_t *)0;
    task->wake_tick = RTOS_WAIT_FOREVER;
...
bool rtos_scheduler_validate(void)
{
    ...
    for (size_t i = 0U; i < rtos_task_registry_count(); ++i)
    {
        ...
        if ((task->state == RTOS_TASK_BLOCKED) && (task->ready_node.owner != (const void *)0))
        {
            return false;
        }
        if ((task->state == RTOS_TASK_SUSPENDED) && ((task->ready_node.owner != (const void *)0) || (task->wait_node.owner != (const void *)0)
            || (task->timeout_node.owner != (const void *)0)))
        {
            return false;
        }
        if ((task->state != RTOS_TASK_BLOCKED) && ((task->waiting_object != (void *)0) || (task->waiting_list != (rtos_list_t *)0)))
        {
            return false;
        }
        if (!rtos_list_validate(&task->owned_mutexes))
        {
            return false;
        }
    }
