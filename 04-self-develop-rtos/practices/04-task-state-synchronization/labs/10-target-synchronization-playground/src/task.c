...
{
    uint32_t *sp;
    if ((task == (rtos_task_t *)0) || (name == (const char *)0) ||
        (entry == (rtos_task_entry_t)0) || (stack == (uint32_t *)0) ||
    ...
    for (size_t i = 0U; i < stack_word_count; ++i) { stack[i] = RTOS_TASK_STACK_FILL; }
...
bool rtos_task_is_valid(const rtos_task_t *task)
{
    return (task != (const rtos_task_t *)0) &&
           ...
           (task->entry != (rtos_task_entry_t)0) &&
           rtos_task_saved_sp_is_valid(task);
}
...
size_t rtos_task_stack_unused_words(const rtos_task_t *task)
{
    ...
    if (!rtos_task_is_valid(task)) { return 0U; }
...
bool rtos_task_transition_allowed(rtos_task_state_t from, rtos_task_state_t to)
{
    switch (from)
    {
        ...
        case RTOS_TASK_RUNNING:
            return (to == RTOS_TASK_READY) || (to == RTOS_TASK_BLOCKED) ||
                   (to == RTOS_TASK_SUSPENDED);
        case RTOS_TASK_BLOCKED:
            return (to == RTOS_TASK_READY) || (to == RTOS_TASK_SUSPENDED);
