...
rtos_task_t *rtos_task_registry_at(size_t index)
{
    ...
    while (node != (rtos_list_node_t *)0)
    {
        if (current == index)
        {
            return RTOS_CONTAINER_OF(node, rtos_task_t, all_task_node);
        }
        node = node->next;
        ++current;
    }
...
rtos_task_t *rtos_task_registry_find(const char *name)
{
    for (size_t i = 0U; i < rtos_task_registry_count(); ++i)
    {
        ...
        if ((task != (rtos_task_t *)0) && strings_equal(task->name, name))
        {
            return task;
        }
    }
...
{
    if (!rtos_list_validate(&g_all_tasks))
    {
        return false;
    }
    for (size_t i = 0U; i < rtos_task_registry_count(); ++i)
    {
        if (!rtos_task_is_valid(rtos_task_registry_at(i)))
        {
            return false;
        }
    }
