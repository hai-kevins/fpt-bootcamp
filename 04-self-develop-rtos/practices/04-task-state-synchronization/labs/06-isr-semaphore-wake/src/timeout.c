...
void rtos_timeout_init(void) { rtos_list_init(&g_delayed_tasks); }
...
bool rtos_timeout_insert(rtos_task_t *task)
{
    ...
    if (!rtos_task_is_valid(task) || (task->wake_tick == RTOS_WAIT_FOREVER) ||
...
{
    return rtos_task_is_valid(task) &&
           (task->timeout_node.owner == &g_delayed_tasks) &&
           rtos_list_remove(&g_delayed_tasks, &task->timeout_node);
}
...
rtos_task_t *rtos_timeout_front(void)
{
    ...
    return (node != (rtos_list_node_t *)0) ?
           RTOS_CONTAINER_OF(node, rtos_task_t, timeout_node) : (rtos_task_t *)0;
}
...
size_t rtos_timeout_count(void) { return rtos_list_count(&g_delayed_tasks); }
const rtos_list_t *rtos_timeout_list(void) { return &g_delayed_tasks; }

bool rtos_timeout_validate(void)
{
    const rtos_list_node_t *node;
    const rtos_task_t *previous = (const rtos_task_t *)0;
    if (!rtos_list_validate(&g_delayed_tasks)) { return false; }
    ...
    while (node != (const rtos_list_node_t *)0)
    {
        ...
        if (!rtos_task_is_valid(task) || (task->state != RTOS_TASK_BLOCKED) ||
            (task->wake_tick == RTOS_WAIT_FOREVER)) { return false; }
        if ((previous != (const rtos_task_t *)0) &&
            rtos_tick_before(task->wake_tick, previous->wake_tick)) { return false; }
        previous = task;
        node = node->next;
    }
    return true;
