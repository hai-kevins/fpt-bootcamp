...
bool app_sm_dispatch(app_sm_t *app, const event_t *event)
{
    ...
    {
        case APP_STATE_IDLE:
            if (event->signal == EVENT_SIGNAL_APP_START)
            {
        ...
        case APP_STATE_RUNNING:
            if (event->signal == EVENT_SIGNAL_APP_STOP)
            {
                app->state = APP_STATE_IDLE;
                app->transition_count++;
                return true;
            ...
            if ((event->signal == EVENT_SIGNAL_APP_FAIL) ||
                (event->signal == EVENT_SIGNAL_APP_TIMEOUT))
            {
        ...
        case APP_STATE_ERROR:
            if (event->signal == EVENT_SIGNAL_APP_RESET)
            {
                app->state = APP_STATE_IDLE;
                app->transition_count++;
                return true;
