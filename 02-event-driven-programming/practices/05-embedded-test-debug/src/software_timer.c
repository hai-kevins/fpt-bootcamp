...
{
    if ((service == NULL) || (event == NULL) || (timer_id >= SOFTWARE_TIMER_MAX_COUNT) || (delay_ms == 0U))
    {
        return false;
    }

    software_timer_entry_t *entry = &service->entries[timer_id];
...
{
    if ((service == NULL) || (timer_id >= SOFTWARE_TIMER_MAX_COUNT))
    {
        return false;
    }
...
void software_timer_process(software_timer_service_t *service, uint32_t now_ms, event_queue_t *queue)
{
    ...
    {
        software_timer_entry_t *entry = &service->entries[i];

        if ((!entry->active) || (now_ms < entry->deadline_ms))
        {
            continue;
        }
