...
int main(void)
{
    ...
    (void)printf(
        "allocate=%s exhaustion=%s reuse=%s "
        "double_free=%s hwm=%zu\n",
        allocate_pass ? "PASS" : "FAIL",
        exhaustion_pass ? "PASS" : "FAIL",
        reuse_pass ? "PASS" : "FAIL",
        double_free_pass ? "PASS" : "FAIL",
        pool.high_water
    );

    return (allocate_pass && exhaustion_pass &&
            reuse_pass && double_free_pass &&
            (pool.current == 0U) &&
            (pool.high_water == 4U)) ? 0 : 1;
}
