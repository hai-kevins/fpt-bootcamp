...
int main(void)
{
    ...
    const size_t oldest_index = (trace.head + CAPACITY - trace.count) % CAPACITY;
    const uint32_t oldest = trace.records[oldest_index].time;

    const bool before_clear = (trace.count == CAPACITY) && (trace.overwritten == 5U) && (oldest == 5U);

    clear(&trace);
    const bool clear_pass = (trace.count == 0U) && (trace.overwritten == 0U);

    (void) printf("count=128 overwritten=5 oldest=5 clear=%s\n", clear_pass ? "PASS" : "FAIL");
