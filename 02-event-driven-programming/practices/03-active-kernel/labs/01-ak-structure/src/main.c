...
typedef struct { uint8_t id, priority, queue_depth; } task_t;
typedef struct { uint16_t signal; uint8_t kind, length, ref_count; } message_t;
typedef struct { uint8_t active, periodic; uint32_t deadline, period; } timer_t;
