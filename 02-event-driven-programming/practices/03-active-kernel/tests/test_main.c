...
int main(void)
{
    const test_entry_t tests[] = {
        ...
        {"task_scheduler", test_task_scheduler},
        {"timer", test_timer},
        {"fsm_and_tsm", test_fsm_and_tsm},
        {"event_record", test_event_record},
        {"fatal_record", test_fatal_record},
        {"application_flow", test_application_flow}
    };
    unsigned passed = 0U;
    for (unsigned i = 0U; i < sizeof(tests)/sizeof(tests[0]); i++)
    {
        if (tests[i].fn()) { printf("[PASS] %s\n", tests[i].name); passed++; }
        else { printf("[FAIL] %s\n", tests[i].name); }
    }
