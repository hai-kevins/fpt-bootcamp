...
{
    ak_message_t *message = ak_message_create_common(
        APP_TASK_APPLICATION,
        APP_TASK_LED,
...
static bool app_dispatch(ak_fsm_t *fsm, const ak_message_t *message)
{
    ...
    {
        case APP_STATE_BOOT:
            if (message->signal == APP_SIGNAL_SYSTEM_START)
            {
        ...
        case APP_STATE_IDLE:
            if (message->signal == APP_SIGNAL_BUTTON_PRESSED)
            {
                post_blink(500U);
        ...
        case APP_STATE_ERROR:
            if (message->signal == APP_SIGNAL_APP_RESET)
            {
                post_led(APP_SIGNAL_LED_OFF);
                ak_fsm_transition(fsm, APP_STATE_IDLE);
                return true;
