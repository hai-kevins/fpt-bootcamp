...
{
    if ((pubsub == NULL) || (object == NULL) ||
        (signal >= ED_PUBSUB_MAX_SIGNALS))
    {
        return false;
    }
                         ...
                         const ed_event_t *prototype)
{
    ...
    if ((pubsub == NULL) || (prototype == NULL) ||
        (pubsub->pool == NULL) ||
        (prototype->signal >= ED_PUBSUB_MAX_SIGNALS))
    {
        return 0U;
    }
    ...
    {
        ed_active_object_t *object =
            pubsub->subscribers[prototype->signal][i];
        ed_event_t *copy = ed_event_pool_allocate(pubsub->pool,
                                                   prototype->signal,
                                                   prototype->source,
                                                   object->id);
        if (copy == NULL)
        ...
        if (!ed_event_set_payload(copy,
                                  prototype->payload,
                                  prototype->payload_length) ||
            !ed_active_object_post(object, copy))
        {
