...
bool ed_event_pool_owns(const ed_event_pool_t *pool, const ed_event_t *event)
{
    ...
    return (event >= &pool->events[0]) && (event < &pool->events[ED_EVENT_POOL_CAPACITY]);
}

bool ed_event_pool_retain(ed_event_pool_t *pool, ed_event_t *event)
{
    if (!ed_event_pool_owns(pool, event) || ((event->flags & ED_EVENT_FLAG_DYNAMIC) == 0U) || (event->pool_index >= ED_EVENT_POOL_CAPACITY)
        || !pool->used[event->pool_index] || (event->ref_count == UINT8_MAX))
    {
        return false;
    }
...
bool ed_event_pool_release(ed_event_pool_t *pool, ed_event_t *event)
{
    ...
    if (((event->flags & ED_EVENT_FLAG_DYNAMIC) == 0U) || (event->pool_index >= ED_EVENT_POOL_CAPACITY) || !pool->used[event->pool_index]
        || (event->ref_count == 0U))
    {
