...
static void demo_handler(ed_active_object_t *base, const ed_event_t *event)
{
...
bool demo_ao_init(demo_ao_t *object, uint8_t id, uint8_t priority, const char *name)
{
    ...
    return ed_active_object_init(&object->super, id, priority, name, 8U, demo_handler, object);
}
