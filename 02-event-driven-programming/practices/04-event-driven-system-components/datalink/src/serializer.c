                     ...
                     size_t *encoded_length)
{
    if ((frame == NULL) || (buffer == NULL) ||
        (encoded_length == NULL) ||
        (frame->payload_length > ED_EVENT_PAYLOAD_MAX))
    {
        return false;
    }

    const size_t total = ED_FRAME_HEADER_LEN +
    ...
    {
        (void)memcpy(&buffer[ED_FRAME_HEADER_LEN],
                     frame->payload,
                     frame->payload_length);
    }

    const uint16_t crc = ed_crc16_ccitt(buffer,
                                        total - ED_FRAME_CRC_LEN);
                     ...
                     ed_frame_t *frame)
{
    if ((buffer == NULL) || (frame == NULL) ||
        (length < (ED_FRAME_HEADER_LEN + ED_FRAME_CRC_LEN)) ||
        (buffer[0] != ED_FRAME_SOF) ||
        (buffer[1] != ED_FRAME_VERSION))
    {
        return false;
    ...
    const size_t expected = ED_FRAME_HEADER_LEN +
                            payload_length + ED_FRAME_CRC_LEN;
    if ((payload_length > ED_EVENT_PAYLOAD_MAX) ||
        (length != expected))
    {
        return false;
    }
    ...
    const uint16_t received_crc =
        read_u16_le(&buffer[length - ED_FRAME_CRC_LEN]);
    const uint16_t calculated_crc =
        ed_crc16_ccitt(buffer, length - ED_FRAME_CRC_LEN);
                       ...
                       ed_event_t *event)
{
    ...
    ed_event_init_static(event,
                         frame->signal,
                         frame->source_object,
                         frame->destination_object);
    event->flags |= ED_EVENT_FLAG_REMOTE;
    event->sequence = frame->sequence;
    return ed_event_set_payload(event,
                                frame->payload,
                                frame->payload_length);
}
