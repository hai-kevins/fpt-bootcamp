...
static void handler_a(const event_t *event)
{
    g_a_count++;
    printf("A,%u,%lu\n",
           (unsigned int)event->signal,
           (unsigned long)event->parameter);
}
...
static bool dispatch(
    const event_t *event,
    const handler_t *table
)
{
    if ((event == NULL) ||
        (event->destination >= (uint8_t)DEST_COUNT) ||
        (table[event->destination] == NULL))
    {
        g_invalid_count++;
        return false;
    }
...
int main(void)
{
    ...
    printf("counts=%lu,%lu,%lu invalid=%lu\n",
           (unsigned long)g_a_count,
           (unsigned long)g_b_count,
           (unsigned long)g_c_count,
           (unsigned long)g_invalid_count);

    return ((g_a_count == 1UL) &&
            (g_b_count == 1UL) &&
            (g_c_count == 1UL) &&
            (g_invalid_count == 1UL))
               ? 0
