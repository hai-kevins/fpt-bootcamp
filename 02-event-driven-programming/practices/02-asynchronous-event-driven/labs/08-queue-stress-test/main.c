...
int main(void)
{
    ...
    printf("attempts=%lu posted=%lu dispatched=%lu dropped=%lu high=%u\n", (unsigned long) ATTEMPTS, (unsigned long) q.posted,
        (unsigned long) q.dispatched, (unsigned long) q.dropped, (unsigned int) q.high);
    return ((q.posted + q.dropped == ATTEMPTS) && (q.posted == q.dispatched) && (q.high == CAPACITY)) ? 0 : 1;
}
