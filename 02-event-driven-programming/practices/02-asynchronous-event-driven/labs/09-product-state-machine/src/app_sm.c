...
void app_sm_dispatch(app_sm_t *sm, app_event_t event)
{
    ...
    switch (sm->state)
    {
        case APP_BOOT : if (event == APP_SYSTEM_START)
            next = APP_IDLE;
        break;
        case APP_IDLE : if (event == APP_BUTTON_PRESSED)
            next = APP_ACTIVE;
            else if (event == APP_ERROR_DETECTED)
            {
                next = APP_ERROR;
            }
        break;
        case APP_ACTIVE : if (event == APP_BUTTON_PRESSED)
            next = APP_IDLE;
            else if (event == APP_ERROR_DETECTED)
            {
                next = APP_ERROR;
