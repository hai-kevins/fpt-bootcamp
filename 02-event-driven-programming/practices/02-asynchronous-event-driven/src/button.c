...
void button_event_handler(const event_t *event)
{
    ...
    switch ((signal_t) event->signal)
    {
        ...
        {
            (void) software_timer_start(SOFTWARE_TIMER_BUTTON_DEBOUNCE, EVENT_DESTINATION_BUTTON, SIGNAL_BUTTON_DEBOUNCE_TIMEOUT,
                0UL, BUTTON_DEBOUNCE_MS, false, event->timestamp_ms);
            break;
        }
        ...
        case SIGNAL_BUTTON_DEBOUNCE_TIMEOUT :
        {
            ...
            if (current_pressed != g_stable_pressed)
            {
                ...
                (void) dispatcher_post(EVENT_SOURCE_BUTTON, EVENT_DESTINATION_APP, current_pressed ? SIGNAL_BUTTON_PRESSED : SIGNAL_BUTTON_RELEASED,
                0UL, event->timestamp_ms);
            }
            break;
        }
