...
static void post_led(signal_t signal, uint32_t parameter, uint32_t timestamp_ms)
{
    (void) dispatcher_post(EVENT_SOURCE_APP, EVENT_DESTINATION_LED, signal, parameter, timestamp_ms);
}

static void post_uart_status(signal_t signal, uint32_t timestamp_ms)
{
    (void) dispatcher_post(EVENT_SOURCE_APP, EVENT_DESTINATION_UART, signal, 0UL, timestamp_ms);
}
...
void app_event_handler(const event_t *event)
{
    ...
    switch ((signal_t) event->signal)
    {
        case SIGNAL_SYSTEM_START :
        {
            ...
            post_uart_status(SIGNAL_UART_STATUS_IDLE, event->timestamp_ms);
            break;
        }
        ...
        case SIGNAL_BUTTON_PRESSED :
        {
            if (g_state == APP_STATE_IDLE)
            {
                ...
                post_led(SIGNAL_LED_BLINK_START, 500UL, event->timestamp_ms);
                post_uart_status(SIGNAL_UART_STATUS_ACTIVE, event->timestamp_ms);
            }
        ...
        case SIGNAL_ERROR_DETECTED :
        {
            ...
            post_led(SIGNAL_LED_BLINK_START, 100UL, event->timestamp_ms);
            post_uart_status(SIGNAL_UART_STATUS_ERROR, event->timestamp_ms);
            break;
        }
