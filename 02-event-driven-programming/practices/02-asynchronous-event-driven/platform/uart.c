...
static void uart_init(void)
{
    ...
    GPIOA_CRH = (GPIOA_CRH & ~((0xFUL << 4U) | (0xFUL << 8U))) | (0xAUL << 4U) | (0x4UL << 8U);
...
void USART1_IRQHandler(void)
{
    ...
    {
        const uint8_t byte = (uint8_t)(USART1_DR & 0xFFUL);

        (void) dispatcher_post(EVENT_SOURCE_ISR, EVENT_DESTINATION_UART, SIGNAL_UART_RX_BYTE, (uint32_t) byte,
